...
def Tsat(P):
  A = 7.8130; B = 11209; C = 5.249e5
  return 2 * C / ( -B + sqrt(B*B + 4 * A * C - 4 * C * log(P / 1e6))) - 273.15

# sa derivee
def DTsat(P):
  A = 7.8130; B = 11209; C = 5.249e5; Tsk = Tsat(P) + 273.15
...
def HV( T, P):
  Ts = Tsat(P); dT = T - Ts; Cp0 = 910; k = -4.6e-3
...
def NuV(T, P, w, Dh):
  Re = RhoV(T, P) * dabs(w) * Dh / MuV(T); Pr = MuV(T) * DTHV(T, P) / LambdaV(T)
